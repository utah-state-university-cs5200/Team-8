...
from src.communications.messages.constants import MESSAGE_ID_CREATE_GAME_SERVER, MESSAGE_ID_ADD_PLAYER, MESSAGE_ID_GET_GAME_STATE
...
class NewGameResponderConversation(ResponderConversation):
    def __init__(self, incoming_envelope, *args, **kwargs):
        super().__init__(incoming_envelope, *args, **kwargs)
        self._valid_incoming_message_types = {MESSAGE_ID_CREATE_GAME_SERVER, MESSAGE_ID_ADD_PLAYER, MESSAGE_ID_GET_GAME_STATE}

    def _execute_details(self):
        if self.incoming_envelope and self.incoming_envelope.message.message_type_id == MESSAGE_ID_CREATE_GAME_SERVER:
